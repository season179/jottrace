...
fn web_journal_view_reads_sessions_events_and_ingest_errors() {
    ...
    assert!(view.events[0].payload_preview.contains("permissionMode"));
...
fn web_journal_view_selects_session_by_source_and_source_session_id() {
    ...
    assert!(view.events[0].payload_preview.contains("claude"));
...
fn web_server_serves_local_journal_html() {
    ...
    write!(
        stream,
        "GET /?session={CLAUDE_FIXTURE_SESSION_ID}&q=sanitized%20reader HTTP/1.1\r\nHost: 127.0.0.1\r\nConnection: close\r\n\r\n"
    )
