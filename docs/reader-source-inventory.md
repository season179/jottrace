...
| Pi agent (#64 fixture-confirmed) | `~/.pi/agent/sessions/<encoded-cwd>/<timestamp>_<session-id>.jsonl` for top-level sessions; nested subagent runs at `~/.pi/agent/sessions/<encoded-cwd>/<timestamp>_<parent-session-id>/<short>/run-N/session.jsonl` | JSONL session files with `session`, `message`, `model_change`, and `thinking_level_change` events | `session.id`; for top-level files this matches the filename session-id suffix, for nested run files it is read from the first `session` event since the filename stem is the literal `session` | JSONL line number, with top-level timestamps retained as event metadata | Event-chain `parentId` values exist on message, model-change, and thinking-level-change events; nested run files are linked to their parent session via the parent UUID extracted from the run directory's grandparent | Existing JSONL generation logic applies: file size, mtime, fingerprint, and next read offset for append, rewrite, and unchanged-file detection |
